...
flowchart TD
    ...
    J -- nein --> E[weitere Seite mit restlichen missing_chars]
    J -- ja --> K[TTF bauen\nPOST /jobs/{sid}/build-ttf\n{language: lang_new}]
...
```mermaid
flowchart TD
    A[Vorhandener Job + Sprache\nsid, lang bekannt] --> B[Zu ändernde Buchstaben festlegen\nLETTERS_TO_CHANGE z.B. "ABC"]
