...
export function useLoginHandler(opts: { apiUser: AxiosInstance; bootstrap?: BootstrapFn }) {
  ...
  const handleDemoStart = async (
    ...
    try {
      const body: any = {
        roles: Array.isArray(demo.roles) && demo.roles.length ? demo.roles : ['teacher'],
      };
      if (demo.preferredName?.trim()) {
        body.preferred_name = demo.preferredName.trim();
      }
      if (demo.lang) {
        body.lang = demo.lang;
      }

      const startResp = await apiUser.post('/auth/demo/start/', body, {
        withCredentials: true,
      });
      const { access: token } = startResp.data;

      const meResp = await apiUser.get('/users/me/', {
...
export async function fetchDemoStatus(apiUser: AxiosInstance) {
  const resp = await apiUser.get('/auth/demo/status/', {
    withCredentials: true,
  });
  return resp.data; // { has_active_demo: bool, ... }
}
