...
**Nutshell WP** integrates a React-based Single Page Application (SPA) with a Django backend into WordPress via Full Site Editing (FSE) blocks.
...
* 🛠️ An **admin configuration page** is added under “Settings > Nutshell Settings”.
...
* Create a final distributable ZIP (`dist/nutshell-wp.zip`)
...
4. Visit **Settings > Nutshell Settings** to configure the API endpoint.

### 🔸 3. Architecture Overview

```
nutshell-frontend/
├── nutshell-wp/       # 📦 WordPress plugin
│   ├── build/           # 🛠️ Compiled JS/PHP/assets for block registration
│   ├── src/             # 🔧 Source TSX/React code
│   ├── dist/            # ✅ Final zip for plugin install
│   ├── nutshell-wp.php # 📜 Main plugin file
...
* API endpoint configured dynamically via `wp_localize_script()` and passed as `nutshellSettings.apiUrl`
...
- [ ] Add mock mode toggle (via window.nutshellSettings.mockMode)
...
```ts
const useMock = () => window.nutshellSettings?.mockMode === true;
```
